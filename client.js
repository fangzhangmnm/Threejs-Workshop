...
class ClientSide{
    constructor(url){
        ...
        this.socket.on("newEntities",(infos)=>{
            ...
            if(netIds.length>0)
                console.log("downloaded remote entities:",netIds.map(netId=>[netId,this.entitiesNetId.get(netId).localId]));
        });
        ...
        this.socket.on("b",(bufferArray)=>{
            ...
            nidArray.forEach(netId=>{
                var e=client.entitiesNetId.get(netId);
                if(e==null)
                    console.log("received dynamics of unexist entity");
                else{
                    if(e.author!=client.id){
        ...
        this.socket.on("e",(info)=>{
            if(client.gameEventHandler)
                client.gameEventHandler(info);
        });
        this.socket.on("s",(info)=>{
            console.log('s',info)
            if(client.id!=info.id){
                var e=client.entitiesNetId.get(info.nid);
                e.staticData[info.key]=info.value;
            }
        });
    ...
    setStaticDataNow(localId,key,value){
        var e=this.entitiesLocalId.get(localId);
        e.staticData[key]=value;
        if(this.roomId!=null){
            this.socket.emit("s",{nid:e.netId,key:key,value:value});
        }else{
            //do nothing
            //entity should be upload later with updated staticData;
        }
    }
    getStaticData(localId,key){
        var e=this.entitiesLocalId.get(localId);
        return e.staticData[key];
    }
...
class ThreeJSClientUpdator{
    constructor(client,uploadInterval){
        ...
        this.userDataLocalId=new Map();
        this.removeHandler=null;
        var updator=this;
        client.createEntityHandler=(localId,isAuthor,p,q,staticData)=>{
            console.log("createEntityHandler")
            let handler=this.createHandlers.get(staticData.class);
            //if(!handler)handler=this.createHandlers.get("default");
            let threeJSObject=null;
            if(handler)
                threeJSObject=handler(localId,isAuthor,p,q,staticData);
            if(threeJSObject){
                let e=updator.client.getEntity(localId);
                updator.threeJSObjects.set(localId,threeJSObject);
                updator.threeJSObjectsReverse.set(threeJSObject,localId);
            }else{
                console.log("unknown staticData.class: ",staticData.class);
            }
        };
        let processEvent=(info)=>{
            let handler=this.gameEventHandlers.get(info.class);
            //if(!handler)handler=this.gameEventListeners.get("default");
            if(handler)handler(info);
        }
        client.gameEventHandler=(info)=>{
            if(Array.isArray(info))
                info.forEach(processEvent);
            else
                processEvent(info);
        }
        client.removeEntityHandler=(localId)=>{
    ...
    create(p,q,className,staticData){
        staticData.class=className;
    ...
    emitGameEvent(info){
        this.client.emit("e",info);
    }
    getNetId(localId){
        let e=this.client.entitiesLocalId.get(localId);
        return e && e.netId;
    }
    getLocalId(netId){
        let e=this.client.entitiesNetId.get(netId);
        return e && e.localId;
    }
    getId(){
    ...
    getThreeJsObjectFromNetId(netId){
        return this.threeJSObjects.get(this.getLocalId(netId));
    }
}

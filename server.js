...
class Room{
    constructor(id,name,server){
        this.id=id;
        this.internalRoomName=this.id.toString();
        //this.broadcaster=server.io.to(this.internalRoomName);这么做是错误的
        this.name=name;
        this.server=server;
    ...
    joinClient(client){
        ...
        client.connection.join(this.internalRoomName);//必须最后再加入房间，否则在完成初始化之前就会收到广播，就乱了
    }
    getRoomTime(){
    ...
    leaveClientAndRemoveEntities(client){
        if(this.joinedClients.has(client.id)){
            ...
            client.connection.leave(this.internalRoomName);
    ...
    createEntity(localId,author,p,q,t,staticData){
        ...
        this.server.io.to(this.internalRoomName).emit("newEntities",[{
            nid:entity.netId,
            lid:entity.localId,
            a:entity.author,
            p:entity.p,
    ...
    removeEntities(netIds,author){
        ...
        this.server.io.to(this.internalRoomName).emit("removeEntities",actuallyRemoved);
    ...
    broadcastImmediately(nidArray,dArray,n0,w,author){
        ...
        if(n1>0){
            ...
            this.server.io.to(this.internalRoomName).emit("b",buffer1);
        }
    }
    broadcastDynamics(){
        ...
        if(n>0){
            ...
            this.server.io.to(this.internalRoomName).emit("b",buffer);
        //console.log(`${this.name}: Broadcasted ${n} entity dynamics to ${this.joinedClients.size} clients packed in ${buffer.byteLength} bytes`);
        }
    }
    updateStatic(author,nid,key,value){
        var entity=this.entities.get(nid);
        if(entity.author==author){
            entity.staticData[key]=value;
            /*this.joinedClients.forEach((client,clientId)=>{
                client.connection.emit("s",{a:author,nid:nid,key:key,value:value});
            });*/
            this.server.io.to(this.internalRoomName).emit("s",{a:author,nid:nid,key:key,value:value});
        }
    }
    broadcastGameEvent(info){
        /*this.joinedClients.forEach((client,clientId)=>{
            client.connection.emit("e",info);
        });*/
        this.server.io.to(this.internalRoomName).emit("e",info);
        //this.broadcaster.emit("e","233");
    }
}
class Server{
    constructor(){
        ...
        this.maxGameEventLength=4096;
        this.maxStaticDataUpdateLength=4096;
        ...
        this.io.on("connection",(socket)=>{
            ...
            socket.on("login",(info)=>{
                ...
                socket.on("e",(info)=>{
                    if(JSON.stringify(info).length>this.maxGameEventLength){
                        //socket.emit("log",`GameEvent request length should be limited in ${server.maxGameEventLength} bytes`);
                        return;
                    }
                    var room=server.rooms.get(client.roomId);
                    if(room!=null)
                        room.broadcastGameEvent(info);
                });
                socket.on("s",(info)=>{
                    if(JSON.stringify(info).length>this.maxStaticDataUpdateLength){
                        //socket.emit("log",`StaticData request length should be limited in ${server.maxStaticDataUpdateLength} bytes`);
                        return;
                    }
                    var room=server.rooms.get(client.roomId);
                    if(room!=null)
                        room.updateStatic(client.id,info.nid,info.key,info.value);
                });
